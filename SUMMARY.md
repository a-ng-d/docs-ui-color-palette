# Table of contents

* [Welcome aboard!](README.md)
* [What is the LCH model?](what-is-the-lch-model.md)

## Create a color palette

* [Overview](create-a-color-palette/overview.md)
* [Select the starting colors](create-a-color-palette/select-the-starting-colors.md)
* [Choose a lightness scale preset](create-a-color-palette/choose-a-lightness-scale-preset.md)
* [Set the lightness scale stops](create-a-color-palette/set-the-lightness-scale-stops.md)
* [Generate the color palette](create-a-color-palette/generate-the-color-palette.md)

## Edit a color palette

* [Overview](edit-a-color-palette/overview.md)
* [Tweak the lightness stops](edit-a-color-palette/tweak-the-lightness-stops.md)
* [Change the starting colors](edit-a-color-palette/change-the-starting-colors.md)

## Deploy a color palette

* [Turn the palette into local styles](deploy-a-color-palette/turn-the-palette-into-local-styles.md)
* [Export the palette](deploy-a-color-palette/export-the-palette.md)

***

* [Troubleshooting](troubleshooting.md)
* [Release notes](release-notes/README.md)
  * [UI Color Palette 19](release-notes/ui-color-palette-19.md)
